...
public class DNSUtil {
	...
	private static final Map<String, DNSUtil.HostAddress> ccache = new Cache(100, 1000 * 60 * 10);
	@SuppressWarnings("unchecked")
	private static final Map<String, DNSUtil.HostAddress> scache = new Cache(100, 1000 * 60 * 10);

	private static DNSUtil.HostAddress resolveSRV(String domain) {
		...
		return new DNSUtil.HostAddress(bestHost, bestPort);
	}
	...
	public static DNSUtil.HostAddress resolveXMPPDomain(String domain) {
		...
		synchronized (DNSUtil.ccache) {
			if (DNSUtil.ccache.containsKey(domain)) {
				DNSUtil.HostAddress address = DNSUtil.ccache.get(domain);
				if (address != null) {
					return address;
				}
			}
		}
		DNSUtil.HostAddress result = DNSUtil.resolveSRV("_xmpp-client._tcp." + domain);
		if (result == null) {
			result = DNSUtil.resolveSRV("_jabber._tcp." + domain);
		}
		if (result == null) {
			result = new DNSUtil.HostAddress(domain, 5222);
		}
		// Add item to cache.
		synchronized (DNSUtil.ccache) {
            DNSUtil.ccache.put(domain, result);
		}
		return result;
	}
	...
	public static DNSUtil.HostAddress resolveXMPPServerDomain(String domain) {
		// Return item from cache if it exists.
		synchronized (DNSUtil.scache) {
			if (DNSUtil.scache.containsKey(domain)) {
				DNSUtil.HostAddress address = DNSUtil.scache.get(domain);
				if (address != null) {
					return address;
				}
			}
		}
		DNSUtil.HostAddress result = DNSUtil.resolveSRV("_xmpp-server._tcp." + domain);
		if (result == null) {
			result = DNSUtil.resolveSRV("_jabber._tcp." + domain);
		}
		if (result == null) {
			result = new DNSUtil.HostAddress(domain, 5269);
		}
		// Add item to cache.
		synchronized (DNSUtil.scache) {
            DNSUtil.scache.put(domain, result);
		}
		return result;
	}
	...
	public static class HostAddress {
		...
		public boolean equals(Object o) {
			...
			if (!(o instanceof DNSUtil.HostAddress)) {
				return false;
			}

			DNSUtil.HostAddress address = (DNSUtil.HostAddress) o;
