...
public class StringUtils {
    ...
    public static String escapeForXML(String string) {
        ...
        for (; i < len; i++) {
            ...
            if (ch > '>') {
                ...
                last = i + 1;
                out.append(StringUtils.QUOTE_ENCODE);
            }
            else if (ch == '\'') {
                if (i > last) {
                    out.append(input, last, i - last);
                ...
                last = i + 1;
                out.append(StringUtils.APOS_ENCODE);
            }
        }
        if (last == 0) {
            return string;
    ...
    public static synchronized String hash(String data) {
        if (StringUtils.digest == null) {
            try {
                StringUtils.digest = MessageDigest.getInstance("SHA-1");
            }
        ...
        // Now, compute hash.
        try {
            StringUtils.digest.update(data.getBytes("UTF-8"));
        }
        catch (UnsupportedEncodingException e) {
            System.err.println(e);
        }
        return StringUtils.encodeHex(StringUtils.digest.digest());
    }
    ...
    public static String randomString(int length) {
        ...
        for (int i=0; i<randBuffer.length; i++) {
            randBuffer[i] = StringUtils.numbersAndLetters[StringUtils.randGen.nextInt(71)];
        }
