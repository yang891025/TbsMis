    ...
    extends SocketFactory
{
	...
	public Socket createSocket(String host, int port) 
        throws IOException {
        return this.httpProxifiedSocket(host, port);
    }
	...
	public Socket createSocket(InetAddress host, int port)
        throws IOException
    {
        return this.httpProxifiedSocket(host.getHostAddress(), port);
	...
	public Socket createSocket( InetAddress address, int port, 
        ...
        throws IOException
    {
        return this.httpProxifiedSocket(address.getHostAddress(), port);
    }
        ...
        throws IOException 
    {
        ...
        while (true)
        {
            ...
            if (got.length() > 1024)
            {
                throw new ProxyException(ProxyType.HTTP, "Recieved " +
        ...
        if (nlchars != 4)
        {
            throw new ProxyException(ProxyType.HTTP, "Never " +
        ...
        if (response == null)
        {
            throw new ProxyException(ProxyType.HTTP, "Empty proxy " +
                "response from " + proxyhost + ", cancelling");
        }
        
        Matcher m = HTTPProxySocketFactory.RESPONSE_PATTERN.matcher(response);
        if (!m.matches())
        {
            throw new ProxyException(ProxyType.HTTP , "Unexpected " +
