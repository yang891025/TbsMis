 ...
 * <li>{@link Roster.SubscriptionMode#accept_all accept_all} -- accept all subscription requests.</li>
 * <li>{@link Roster.SubscriptionMode#reject_all reject_all} -- reject all subscription requests.</li>
 * <li>{@link Roster.SubscriptionMode#manual manual} -- manually process all subscription requests.</li>
 * </ul>
 *
 * @author Matt Tucker
...
public class Roster {
    ...
    private final Roster.PresencePacketListener presencePacketListener;
     ...
     * is {@link Roster.SubscriptionMode#accept_all}.
     *
    ...
    public static Roster.SubscriptionMode getDefaultSubscriptionMode() {
        return Roster.defaultSubscriptionMode;
    }
    ...
    public static void setDefaultSubscriptionMode(Roster.SubscriptionMode subscriptionMode) {
        Roster.defaultSubscriptionMode = subscriptionMode;
    }

    Roster(Connection connection, RosterStorage persistentStorage){
    	this(connection);
    ...
    Roster(Connection connection) {
        ...
        this.presencePacketListener = new Roster.PresencePacketListener();
     ...
     * The default subscription mode is {@link Roster.SubscriptionMode#accept_all}.<p>
     * <p/>
    ...
    public void reload() {
    	...
    	PacketFilter idFilter = new PacketIDFilter(this.requestPacketId);
        this.connection.addPacketListener(new RosterResultListener(), idFilter);
    ...
    private void insertRosterItems(List<Item> items){
    	 Collection<String> addedEntries = new ArrayList<String>();
         Collection<String> updatedEntries = new ArrayList<String>();
         Collection<String> deletedEntries = new ArrayList<String>();
         Iterator<Item> iter = items.iterator();
         while(iter.hasNext()){
             this.insertRosterItem(iter.next(), addedEntries,updatedEntries,deletedEntries);
         }
        this.fireRosterChangedEvent(addedEntries, updatedEntries, deletedEntries);
    }

    private void insertRosterItem(Item item, Collection<String> addedEntries,
    		Collection<String> updatedEntries, Collection<String> deletedEntries){
    	RosterEntry entry = new RosterEntry(item.getUser(), item.getName(),
                item.getItemType(), item.getItemStatus(), this, this.connection);
     ...
     * that's {@link Mode#chat free to chat},
     * {@link Mode#available available},
     * {@link Mode#away away},
     * {@link Mode#xa extended away}, and
     * {@link Mode#dnd do not disturb}.<p>
     * <p/>
		...
		public void processPacket(Packet packet) {
            ...
            if (presence.getType() == Presence.Type.available) {
                ...
                if (Roster.this.presenceMap.get(key) == null) {
                    ...
                    userPresences = Roster.this.presenceMap.get(key);
                }
                ...
                RosterEntry entry = Roster.this.entries.get(key);
                if (entry != null) {
            ...
            // If an "unavailable" packet.
                ...
                // Otherwise, this is a normal offline presence.
                    ...
                    Map<String, Presence> userPresences = Roster.this.presenceMap.get(key);
    ...
    private class RosterResultListener implements PacketListener{
		...
		public void processPacket(Packet packet) {
			if(packet instanceof IQ){
				IQ result = (IQ)packet;
				if(result.getType().equals(Type.RESULT) && result.getExtensions().isEmpty()){
					Collection<String> addedEntries = new ArrayList<String>();
		            Collection<String> updatedEntries = new ArrayList<String>();
		            Collection<String> deletedEntries = new ArrayList<String>();
		            if(Roster.this.persistentStorage !=null){
		            	for(Item item : Roster.this.persistentStorage.getEntries()){
    ...
    private class RosterPacketListener implements PacketListener {
		...
		public void processPacket(Packet packet) {
            ...
            List<Item> rosterItems = new ArrayList<Item>();
            ...
            if(Roster.this.persistentStorage !=null){
            	for (Item i : rosterPacket.getRosterItems()){
            		if(i.getItemType().equals(ItemType.remove)){
