...
public class SASLAuthentication implements UserAuthentication {
    ...
    private static final Map<String, Class> implementedMechanisms = new HashMap<String, Class>();
    private static final List<String> mechanismsPreferences = new ArrayList<String>();
...
//        supportSASLMechanism("CRAM-MD5",2);
    ...
    public static void registerSASLMechanism(String name, Class mClass) {
        SASLAuthentication.implementedMechanisms.put(name, mClass);
    }
    ...
    public static void unregisterSASLMechanism(String name) {
        SASLAuthentication.implementedMechanisms.remove(name);
        SASLAuthentication.mechanismsPreferences.remove(name);
    }
    ...
    public static void supportSASLMechanism(String name) {
        SASLAuthentication.mechanismsPreferences.add(0, name);
    }
    ...
    public static void supportSASLMechanism(String name, int index) {
        SASLAuthentication.mechanismsPreferences.add(index, name);
    }
    ...
    public static List<Class> getRegisterSASLMechanisms() {
        ...
        for (String mechanismsPreference : SASLAuthentication.mechanismsPreferences) {
            answer.add(SASLAuthentication.implementedMechanisms.get(mechanismsPreference));
        }
        return answer;
    }
	...
	public String authenticate(String username, String resource, CallbackHandler cbh) 
        ...
        for (String mechanism : SASLAuthentication.mechanismsPreferences) {
            if (SASLAuthentication.implementedMechanisms.containsKey(mechanism) &&
                    this.serverMechanisms.contains(mechanism)) {
        ...
        if (selectedMechanism != null) {
            ...
            try {
                Class mechanismClass = SASLAuthentication.implementedMechanisms.get(selectedMechanism);
                ...
                synchronized (this) {
                    ...
                    while (!this.saslNegotiated && !this.saslFailed && System.currentTimeMillis() < endTime) {
                        try {
                            this.wait(Math.abs(System.currentTimeMillis() - endTime));
                        }
                ...
                if (this.saslFailed) {
                    ...
                    if (this.errorCondition != null) {
                        throw new XMPPException("SASL authentication " +
                                selectedMechanism + " failed: " + this.errorCondition);
                    }
                    else {
        ...
        if (selectedMechanism != null) {
            ...
            try {
                ...
                if (this.saslNegotiated) {
                    ...
                    return new NonSASLAuthentication(this.connection)
    ...
    private String bindResourceAndEstablishSession(String resource) throws XMPPException {
        ...
        synchronized (this) {
            ...
            while (!this.resourceBinded && System.currentTimeMillis() < endTime) {
                try {
                    this.wait(Math.abs(System.currentTimeMillis() - endTime));
                }
                catch (InterruptedException e) {
