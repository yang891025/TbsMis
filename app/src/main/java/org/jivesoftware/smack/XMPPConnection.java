...
public class XMPPConnection extends Connection {
	...
	public synchronized void login(String username, String password,
		...
		String response;
		if (this.config.isSASLAuthenticationEnabled()
				&& this.saslAuthentication.hasNonAnonymousAuthentication()) {
			// Authenticate using SASL
			if (password != null) {
				response = this.saslAuthentication.authenticate(username, password,
						resource);
			} else {
				response = this.saslAuthentication.authenticate(username, resource,
                        this.config.getCallbackHandler());
			}
		} else {
		...
		if (response != null) {
            ...
            user = username + "@" + this.getServiceName();
			if (resource != null) {
                user += "/" + resource;
			}
		}
		...
		if (roster == null) {
			if (this.rosterStorage == null) {
                ...
                roster = new Roster(this, this.rosterStorage);
			}
		}
	...
	public synchronized void loginAnonymously() throws XMPPException {
		...
		String response;
		if (this.config.isSASLAuthenticationEnabled()
				&& this.saslAuthentication.hasAnonymousAuthentication()) {
	...
	private void connectUsingConfiguration(ConnectionConfiguration config)
		...
		try {
			if (config.getSocketFactory() == null) {
                ...
                socket = config.getSocketFactory()
			...
			throw new XMPPException(errorMessage, new XMPPError(
					Condition.remote_server_timeout, errorMessage),
					uhe);
		} catch (IOException ioe) {
			String errorMessage = "XMPPError connecting to " + host + ":"
					+ port + ".";
			throw new XMPPException(errorMessage, new XMPPError(
					Condition.remote_server_error, errorMessage), ioe);
		}
        this.initConnection();
	}
	...
	private void initConnection() throws XMPPException {
		...
		try {
			if (isFirstInitialization) {
				...
				if (this.config.isDebuggerEnabled()) {
					...
					if (this.debugger.getWriterListener() != null) {
                        this.addPacketSendingListener(this.debugger.getWriterListener(),
								null);
					}
				}
			} else {
			...
			if (isFirstInitialization) {
				...
				for (ConnectionCreationListener listener : Connection.getConnectionCreationListeners()) {
	...
	private void initReaderAndWriter() throws XMPPException {
		try {
			if (!this.usingCompression) {
				...
				try {
					...
					Object in = constructor
							.newInstance(this.socket.getInputStream());
			...
			throw new XMPPException(
					...
					new XMPPError(Condition.remote_server_error,
							"XMPPError establishing connection with server."),
					ioe);
		}
	...
	void startTLSReceived(boolean required) {
		if (required
				&& this.config.getSecurityMode() == SecurityMode.disabled) {
            this.packetReader
		...
		if (this.config.getSecurityMode() == SecurityMode.disabled) {
	...
	void proceedTLSReceived() throws Exception {
		...
		if (this.config.getCallbackHandler() == null) {
			...
			if (this.config.getKeystoreType().equals("NONE")) {
				...
				try {
					...
					ks.load(new FileInputStream(this.config.getKeystorePath()),
							pcb.getPassword());
				} catch (Exception e) {
					ks = null;
					pcb = null;
		...
		context.init(kms,
				new TrustManager[] { new ServerTrustManager(
                        this.getServiceName(), this.config) },
				new SecureRandom());
	...
	private boolean hasAvailableCompressionMethod(String method) {
		return this.compressionMethods != null
				&& this.compressionMethods.contains(method);
	}
		...
		if (this.connected && this.wasAuthenticated) {
			...
			try {
				if (this.isAnonymous()) {
                    ...
                    this.login(this.config.getUsername(), this.config.getPassword(),
	...
	public void startKeepAliveThread(XmppManager xmppManager) throws Exception {
        this.packetWriter.startKeepAliveProcess(xmppManager);
	}
}
