...
public final class PrivateCredentialPermission extends Permission {
    ...
    public PrivateCredentialPermission(String name, String action) {
        super(name);
        if (PrivateCredentialPermission.READ.equalsIgnoreCase(action)) {
            this.initTargetName(name);
        } else {
    ...
    PrivateCredentialPermission(String credentialClass, Set<Principal> principals) {
        ...
        this.set = new PrivateCredentialPermission.CredOwner[principals.size()];
        for (Principal p : principals) {
            PrivateCredentialPermission.CredOwner element = new PrivateCredentialPermission.CredOwner(p.getClass().getName(), p.getName());
            // check for duplicate elements
            boolean found = false;
            for (int ii = 0; ii < this.offset; ii++) {
                if (this.set[ii].equals(element)) {
                    found = true;
                    break;
                }
            }
            if (!found) {
                this.set[this.offset++] = element;
            }
        }
    }
    ...
    private void initTargetName(String name) {
        ...
        this.set = new PrivateCredentialPermission.CredOwner[count];
        for (int index = 0, i, j; index < count; beg = j + 2, index++) {
            ...
            PrivateCredentialPermission.CredOwner element = new PrivateCredentialPermission.CredOwner(principalClass, principalName);
            // check for duplicate elements
            boolean found = false;
            for (int ii = 0; ii < this.offset; ii++) {
                if (this.set[ii].equals(element)) {
    ...
    public String[][] getPrincipals() {
        ...
        String[][] s = new String[this.offset][2];
    ...
    public String getActions() {
        return PrivateCredentialPermission.READ;
    }
    ...
    public int hashCode() {
        ...
        for (int i = 0; i < this.offset; i++) {
            hash = hash + this.set[i].hashCode();
        }
        return this.getCredentialClass().hashCode() + hash;
    }
    ...
    public boolean equals(Object obj) {
        ...
        return this.credentialClass.equals(that.credentialClass) && this.offset == that.offset
                && this.sameMembers(this.set, that.set, this.offset);
    }
    ...
    public boolean implies(Permission permission) {
        ...
        PrivateCredentialPermission.CredOwner[] thisCo = this.set;
        PrivateCredentialPermission.CredOwner[] thatCo = that.set;
    ...
    private static final class CredOwner implements Serializable {
        ...
        boolean implies(Object obj) {
            ...
            PrivateCredentialPermission.CredOwner co = (PrivateCredentialPermission.CredOwner) obj;

            if (this.isClassWildcard || this.principalClass.equals(co.principalClass)) {
                if (this.isPNameWildcard || this.principalName.equals(co.principalName)) {
                    return true;
                }
            }
            return false;
        ...
        public boolean equals(Object obj) {
            ...
            if (obj instanceof PrivateCredentialPermission.CredOwner) {
                PrivateCredentialPermission.CredOwner that = (PrivateCredentialPermission.CredOwner) obj;
                return this.principalClass.equals(that.principalClass)
