...
parseV6(String s) {
	...
	for (i = first, j = 0; i <= last; i++) {
		...
		if (tokens[i].indexOf('.') >= 0) {
			...
			if (i > 6)
				return null;
			byte [] v4addr = toByteArray(tokens[i], Address.IPv4);
			if (v4addr == null)
				return null;
			for (int k = 0; k < 4; k++)
				data[j++] = v4addr[k];
...
public static int []
toArray(String s) {
	return Address.toArray(s, Address.IPv4);
}
...
isDottedQuad(String s) {
	byte [] address = toByteArray(s, Address.IPv4);
	return address != null;
}
...
getByAddress(String addr) throws UnknownHostException {
	byte [] bytes;
	bytes = Address.toByteArray(addr, Address.IPv4);
	if (bytes != null)
		return InetAddress.getByAddress(bytes);
	bytes = Address.toByteArray(addr, Address.IPv6);
	if (bytes != null)
		return InetAddress.getByAddress(bytes);
...
getByAddress(String addr, int family) throws UnknownHostException {
	if (family != Address.IPv4 && family != Address.IPv6)
		throw new IllegalArgumentException("unknown address family");
	byte [] bytes;
	bytes = Address.toByteArray(addr, family);
	if (bytes != null)
		return InetAddress.getByAddress(bytes);
	throw new UnknownHostException("Invalid address: " + addr);
}
