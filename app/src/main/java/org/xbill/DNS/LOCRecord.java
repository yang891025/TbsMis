...
static {
    LOCRecord.w2 = new DecimalFormat();
    LOCRecord.w2.setMinimumIntegerDigits(2);

    LOCRecord.w3 = new DecimalFormat();
    LOCRecord.w3.setMinimumIntegerDigits(3);
}
...
throws IOException
{
	...
	try {
		long value = (long)(100 * this.parseFixedPoint(s));
...
rdataFromString(Tokenizer st, Name origin) throws IOException {
    ...
    this.altitude = this.parseDouble(st, "altitude", true,
			       -10000000, 4284967295L, 0) + 10000000;
    this.size = this.parseDouble(st, "size", false, 0, 9000000000L, 100);
    this.hPrecision = this.parseDouble(st, "horizontal precision", false,
				 0, 9000000000L, 1000000);
    this.vPrecision = this.parseDouble(st, "vertical precision", false,
				 0, 9000000000L, 1000);
}
...
positionToString(long value, char pos, char neg) {
    ...
    this.renderFixedPoint(sb, LOCRecord.w3, temp, 1000); /* seconds */
	sb.append(" ");
...
rrToString() {
	...
	/* Altitude */
    this.renderFixedPoint(sb, LOCRecord.w2, this.altitude - 10000000, 100);
	sb.append("m ");

	/* Size */
    this.renderFixedPoint(sb, LOCRecord.w2, this.size, 100);
	sb.append("m ");

	/* Horizontal precision */
    this.renderFixedPoint(sb, LOCRecord.w2, this.hPrecision, 100);
	sb.append("m ");

	/* Vertical precision */
    this.renderFixedPoint(sb, LOCRecord.w2, this.vPrecision, 100);
	sb.append("m");
...
public double
getLatitude() {  
	return (double) (this.latitude - (1L << 31)) / (3600 * 1000);
}       

/** Returns the longitude */
public double
getLongitude() {  
	return (double) (this.longitude - (1L << 31)) / (3600 * 1000);
}       

/** Returns the altitude */
public double
getAltitude() {  
	return (double) (this.altitude - 10000000) / 100;
}       

/** Returns the diameter of the enclosing sphere */
public double
getSize() {  
	return (double) this.size / 100;
}       

/** Returns the horizontal precision */
public double
getHPrecision() {  
	return (double) this.hPrecision / 100;
}       

/** Returns the horizontal precision */
public double
getVPrecision() {  
	return (double) this.vPrecision / 100;
}       
