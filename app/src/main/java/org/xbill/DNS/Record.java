...
static {
    Record.byteFormat.setMinimumIntegerDigits(3);
}
...
{
	Record rec;
	rec = Record.getEmptyRecord(name, type, dclass, ttl, in != null);
	if (in != null) {
...
newRecord(Name name, int type, int dclass) {
	return Record.newRecord(name, type, dclass, 0);
}
...
fromWire(DNSInput in, int section, boolean isUpdate) throws IOException {
	...
	rec = Record.newRecord(name, type, dclass, ttl, length, in);
	return rec;
}
...
toString() {
	...
	if (Options.check("BINDTTL"))
		...
		sb.append(this.ttl);
	sb.append("\t");
	if (this.dclass != DClass.IN || !Options.check("noPrintIN")) {
...
throws IOException
{
	...
	st.unget();
	rec = Record.getEmptyRecord(name, type, dclass, ttl, true);
...
sameRRset(Record rec) {
	return this.getRRsetType() == rec.getRRsetType() &&
            this.dclass == rec.dclass &&
