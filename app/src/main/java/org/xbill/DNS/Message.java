...
Message(DNSInput in) throws IOException {
	this(new Header(in));
	boolean isUpdate = this.header.getOpcode() == Opcode.UPDATE;
	boolean truncated = this.header.getFlag(Flags.TC);
	try {
		for (int i = 0; i < 4; i++) {
...
findRRset(Name name, int type) {
	return this.findRRset(name, type, Section.ANSWER) ||
            this.findRRset(name, type, Section.AUTHORITY) ||
            this.findRRset(name, type, Section.ADDITIONAL);
}
...
public Record
getQuestion() {
	List l = this.sections[Section.QUESTION];
	if (l == null || l.size() == 0)
		return null;
...
public TSIGRecord
getTSIG() {
	int count = this.header.getCount(Section.ADDITIONAL);
	if (count == 0)
		return null;
	List l = this.sections[Section.ADDITIONAL];
...
public boolean
isSigned() {
	return this.tsigState == Message.TSIG_SIGNED ||
            this.tsigState == Message.TSIG_VERIFIED ||
            this.tsigState == Message.TSIG_FAILED;
}
...
public boolean
isVerified() {
	return this.tsigState == Message.TSIG_VERIFIED;
}
...
public OPTRecord
getOPT() {
	Record [] additional = this.getSectionArray(Section.ADDITIONAL);
...
toWire(DNSOutput out, int maxLength) {
	...
	for (int i = 0; i < 4; i++) {
		...
		if (this.sections[i] == null)
			continue;
		skipped = this.sectionToWire(out, i, c, tempMaxLength);
		if (skipped != 0) {
	...
	if (this.tsigkey != null) {
		TSIGRecord tsigrec = this.tsigkey.generate(this, out.toByteArray(),
                this.tsigerror, this.querytsig);
...
toString() {
	...
	for (int i = 0; i < 4; i++) {
		if (this.header.getOpcode() != Opcode.UPDATE)
