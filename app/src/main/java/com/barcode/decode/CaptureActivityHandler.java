...
public final class CaptureActivityHandler extends Handler {
	...
	public CaptureActivityHandler(ScanActivity activity,
		...
		// 启动扫描线程
        this.decodeThread = new DecodeThread(activity, decodeFormats, characterSet,
	...
	public void handleMessage(Message message) {
		...
		case R.id.restart_preview: // 准备进行下一次扫描
			Log.d(CaptureActivityHandler.TAG, "Got restart preview message");
            this.restartPreviewAndDecode();
			break;
		case R.id.decode_succeeded:
			Log.d(CaptureActivityHandler.TAG, "Got decode succeeded message");
            this.state = CaptureActivityHandler.State.SUCCESS;
		...
		case R.id.return_scan_result:
			Log.d(CaptureActivityHandler.TAG, "Got return scan result message");
            this.activity.setResult(Activity.RESULT_OK, (Intent) message.obj);
            this.activity.finish();
			break;
		case R.id.launch_product_query:
			Log.d(CaptureActivityHandler.TAG, "Got product query message");
			 ...
			 */
			ResolveInfo resolveInfo = this.activity.getPackageManager()
					.resolveActivity(intent, PackageManager.MATCH_DEFAULT_ONLY);
			String browserPackageName = null;
			if (resolveInfo != null && resolveInfo.activityInfo != null) {
				browserPackageName = resolveInfo.activityInfo.packageName;
				Log.d(CaptureActivityHandler.TAG, "Using browser in package " + browserPackageName);
			}
			...
			try {
				...
				Log.w(CaptureActivityHandler.TAG, "Can't find anything to handle VIEW of URI " + url);
			}
			break;
		}
	}
	...
	public void setDecodeFormats(Vector<BarcodeFormat> paramVector) {
        decodeThread.setDecodeFormats(paramVector);
	}
}
