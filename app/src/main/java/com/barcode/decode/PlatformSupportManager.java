...
public abstract class PlatformSupportManager<T> {
  ...
  protected PlatformSupportManager(Class<T> managedInterface, T defaultImplementation) {
    ...
    this.defaultImplementation = defaultImplementation;
      implementations = new TreeMap<Integer,String>(Collections.reverseOrder());
  }
  ...
  public T build() {
    for (Integer minVersion : this.implementations.keySet()) {
      if (VERSION.SDK_INT >= minVersion) {
        String className = this.implementations.get(minVersion);
        try {
          Class<? extends T> clazz = Class.forName(className).asSubclass(this.managedInterface);
          Log.i(PlatformSupportManager.TAG, "Using implementation " + clazz + " of " + this.managedInterface + " for SDK " + minVersion);
          return clazz.getConstructor().newInstance();
        } catch (ClassNotFoundException cnfe) {
          Log.w(PlatformSupportManager.TAG, cnfe);
        } catch (IllegalAccessException iae) {
          Log.w(PlatformSupportManager.TAG, iae);
        } catch (InstantiationException ie) {
          Log.w(PlatformSupportManager.TAG, ie);
        } catch (NoSuchMethodException nsme) {
          Log.w(PlatformSupportManager.TAG, nsme);
        } catch (InvocationTargetException ite) {
          Log.w(PlatformSupportManager.TAG, ite);
        }
      }
    }
    Log.i(PlatformSupportManager.TAG, "Using default implementation " + this.defaultImplementation.getClass() + " of " + this.managedInterface);
