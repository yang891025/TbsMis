...
public final class GingerbreadOpenCameraInterface implements OpenCameraInterface {
  ...
  public Camera open() {
    ...
    if (numCameras == 0) {
      Log.w(GingerbreadOpenCameraInterface.TAG, "No cameras!");
      return null;
    }
    ...
    Camera camera;
    if (index < numCameras) {
      Log.i(GingerbreadOpenCameraInterface.TAG, "Opening camera #" + index);
      camera = Camera.open(index);
    } else {
      Log.i(GingerbreadOpenCameraInterface.TAG, "No camera facing back; returning camera #0");
