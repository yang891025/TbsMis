...
public class WeiXinSendedActivity extends Activity implements View.OnClickListener
{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        this.title_tv.setText("已发信息");
        this.clearBtn.setOnClickListener(this);
        this.backBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                WeiXinSendedActivity.this.finish();
            }
        });
        ...
        this.lvNews.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view,
                ...
                // 点击头部、底部栏无效
                if (position == 0 || position == WeiXinSendedActivity.this.Userlist.size() + 1)
                    return;

                String sourceUrl = holder.url;
                // 跳转到新闻详情
                Intent intent = new Intent();
                intent.setClass(WeiXinSendedActivity.this,
                        BrowserActivity.class);
                intent.putExtra("tempUrl", sourceUrl);
                WeiXinSendedActivity.this.startActivity(intent);
            }
        });
        this.lvNews.setOnScrollListener(new OnScrollListener() {
            @Override
            public void onScrollStateChanged(AbsListView view, int scrollState) {
                WeiXinSendedActivity.this.lvNews.onScrollStateChanged(view, scrollState);
                // 数据为空--不用继续下面代码了
                if (WeiXinSendedActivity.this.Userlist.isEmpty())
                    return;
                // 判断是否滚动到底部
                boolean scrollEnd = false;
                try {
                    if (view.getPositionForView(WeiXinSendedActivity.this.lvNews_footer) == view
                ...
                int lvDataState = StringUtils.toInt(WeiXinSendedActivity.this.lvNews.getTag());
                if (scrollEnd && lvDataState == UIHelper.LISTVIEW_DATA_MORE) {
                    WeiXinSendedActivity.this.lvNews.setTag(UIHelper.LISTVIEW_DATA_LOADING);
                    WeiXinSendedActivity.this.lvNews_foot_more
                            .setText(R.string.pull_to_refresh_refreshing_label);
                    WeiXinSendedActivity.this.lvNews_foot_progress.setVisibility(View.VISIBLE);
                    // 当前pageIndex
                    // int pageIndex = lvNewsSumData / AppContext.PAGE_SIZE;
                    WeiXinSendedActivity.this.initUserData(2);
                }
            }
            ...
            public void onScroll(AbsListView view, int firstVisibleItem,
                ...
                WeiXinSendedActivity.this.lvNews.onScroll(view, firstVisibleItem, visibleItemCount,
                        totalItemCount);
            }
        });
        this.lvNews.setOnRefreshListener(new OnRefreshListener() {
            ...
            public void onRefresh() {
                WeiXinSendedActivity.this.initUserData(1);
            }
        });
        this.connect(0);
    }
    ...
    private void connect(int count) {
        WeiXinSendedActivity.GetAsyncTask task = new WeiXinSendedActivity.GetAsyncTask(count, this);
        task.execute();
    }
    ...
    class GetAsyncTask extends AsyncTask<Integer, Integer, String> {
        ...
        protected void onPreExecute() {
            ...
            WeiXinSendedActivity.this.startAnimation();
        }
        ...
        protected String doInBackground(Integer... params) {
            if (this.count == 1)
                WeiXinSendedActivity.this.clearData();
            WeiXinSendedActivity.this.initLocalData();
            return "ok";
        }
