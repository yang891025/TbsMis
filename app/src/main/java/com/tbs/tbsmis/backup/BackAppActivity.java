...
public class BackAppActivity extends Activity implements View.OnClickListener
{
    ...
    private class myBroadcastreceiver extends BroadcastReceiver
    {
        ...
        public void onReceive(Context arg0, Intent arg1) {
            ...
            if (action.equalsIgnoreCase("backup")) {
                ...
                BackAppActivity.this.backup_sub_dir.setText(subDir);
            }
        }
    ...
    private void init() {
        ...
        backup_server_dir.setText("商城目录");
        String appCategory = this.m_iniFileIO.getIniString(this.appIniFile, "TBSAPP",
                "AppCategory", "", (byte) 0);
        if (StringUtils.isEmpty(appCategory))
            backup_sub_dir.setText("/资讯应用");
        else {
            backup_sub_dir.setText("/" + appCategory+"/" + appDirName);
        }
        back_content.setText(m_iniFileIO.getIniString(this.appIniFile, "TBSAPP",
                "AppMsg", "", (byte) 0));
        subdirPath.setVisibility(View.VISIBLE);
        this.localPath.setVisibility(View.GONE);
    ...
    private void startBackup() {
        ...
        if (StringUtils.isEmpty(backupdir)) {
            ...
            String subdir = this.backup_sub_dir.getText().toString();
            if (backupdir.equals("商城目录")) {
                if (StringUtils.isEmpty(subdir)) {
                    Toast.makeText(this, "服务子目录不可为空", Toast.LENGTH_SHORT)
                            .show();
                } else {
                    ...
                    if (StringUtils.isEmpty(content)) {
                        ...
                        this.showRinghtsDialog(
                                this.backup_local_dir.getText().toString(), 0);
                    }
                }
            } else {
                String content = this.back_content.getText().toString();
                if (StringUtils.isEmpty(content)) {
                    Toast.makeText(this, "备份描述不可为空", Toast.LENGTH_SHORT).show();
                } else {
                    this.showRinghtsDialog(this.backup_local_dir.getText().toString(), 1);
                }
            }
        }
    }
    ...
    private void initPath() {
        ...
        this.backup_local_dir.setText(webRoot);
        String WebIniFile = webRoot + constants.WEB_CONFIG_FILE_NAME;
        this.appIniFile = webRoot
                + this.m_iniFileIO.getIniString(WebIniFile, "TBSWeb", "IniName",
                constants.NEWS_CONFIG_FILE_NAME, (byte) 0);
        appDirName = webRoot.substring(0,webRoot.length()-1);
        appDirName = appDirName.substring(appDirName.lastIndexOf("/")+1);
    }
                        ...
                        .setItems(items, new DialogInterface.OnClickListener()
                        {
                            ...
                            public void onClick(DialogInterface dialog,
                                ...
                                if (items[which].equalsIgnoreCase("应用名称")) {
                                    BackAppActivity.this.appName.setVisibility(View.VISIBLE);
                                } else if (items[which]
                                        .equalsIgnoreCase("当前版本")) {
                                    BackAppActivity.this.appVer.setVisibility(View.VISIBLE);
                                } else {
                                    Toast.makeText(BackAppActivity.this,
                ...
                new Builder(this).setTitle("请点击选择")
                        ...
                        .setItems(items, new DialogInterface.OnClickListener()
                        {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                BackAppActivity.this.backup_server_dir.setText(items[which]);
                                if (which == 0) {
                                    BackAppActivity.this.subdirPath.setVisibility(View.VISIBLE);
                                } else {
                                    BackAppActivity.this.subdirPath.setVisibility(View.GONE);
                                }
                            }
                        }).show();
                break;
                ...
                UIHelper.showFilePathDialog(this, 1, appPath + "data/",
                        new PathChooseDialog.ChooseCompleteListener()
                        {
                            ...
                            public void onComplete(String finalPath) {
                                ...
                                BackAppActivity.this.backup_local_dir.setText(finalPath);
                            }
                        });
                break;
        }
    ...
    private void showDownloadDialog(String CheckedPath, int serverDir) {
        ...
        builder.setNeutralButton("取消", new DialogInterface.OnClickListener()
        {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                BackAppActivity.setUnzip(false);
                dialog.dismiss();
    ...
    private void showModifyDialog(String FilePath, int count,
        ...
        this.Prodialog.setButton("取消", new DialogInterface.OnClickListener()
        {
            ...
            public void onClick(DialogInterface dialog, int which) {
                // TODO Auto-generated method
                // stub
                BackAppActivity.setUnupdate(false);
                BackAppActivity.this.connection.setUnupdata(false);
                dialog.dismiss();

            }
        });
        BackAppActivity.setUnupdate(true);
        this.connect(FilePath, count, CheckedPath, serverDir);
    }

    private void connect(List<String> CheckedFile, String zipFile, int count,
                         String CheckedPath, int serverDir) {
        BackAppActivity.MyAsyncTask task = new BackAppActivity.MyAsyncTask(CheckedFile, zipFile, count, this,
    ...
    private void connect(String CheckedFile, int count, String CheckedPath,
                         int serverDir) {
        BackAppActivity.MyAsyncTask task = new BackAppActivity.MyAsyncTask(CheckedFile, count, this,
                CheckedPath, serverDir);
        task.execute();
    }
    ...
    class MyAsyncTask extends AsyncTask<Integer, Integer, String>
    {
        ...
        protected String doInBackground(Integer... params) {
            ...
            rootPath = rootPath
                    + BackAppActivity.this.m_iniFileIO.getIniString(WebIniFile, "TBSWeb", "IniName",
                    constants.NEWS_CONFIG_FILE_NAME, (byte) 0);
            String userIni = rootPath;
            if (Integer.parseInt(BackAppActivity.this.m_iniFileIO.getIniString(userIni, "Login",
                    "LoginType", "0", (byte) 0)) == 1) {
            ...
            switch (this.count) {
                case 3:
                    ...
                    return this.connection.asyncConnect(
                            constants.verifyURL,
                            this.UserRights(BackAppActivity.this.m_iniFileIO.getIniString(userIni, "Login",
                                    "LoginId", "", (byte) 0), BackAppActivity.this.m_iniFileIO
                ...
                case 2:
                    ...
                    return this.connection.asyncConnect(
                            constants.verifyURL,
                            this.UpdateFile(BackAppActivity.this.m_iniFileIO.getIniString(userIni, "Login",
                                    "LoginId", "", (byte) 0), BackAppActivity.this.m_iniFileIO
                                    .getIniString(userIni, "Login", "Account", "",
                ...
                case 1:
                    ...
                    param.put("userName", BackAppActivity.this.m_iniFileIO
                            .getIniString(userIni, "Login", "Account", "",
                                    (byte) 0));
                    param.put("login_id", BackAppActivity.this.m_iniFileIO.getIniString(userIni, "Login",
                            "LoginId", "", (byte) 0));
        ...
        protected void onPostExecute(String result) {
            //System.out.println("count = "+ count + "\n"+ "constants.verifyURL" + constants.verifyURL);
            if (result == null) {
                if (BackAppActivity.this.Prodialog.isShowing()) {
                    BackAppActivity.this.Prodialog.dismiss();
                }
                if (BackAppActivity.this.ModifyDialog.isShowing()) {
                    BackAppActivity.this.ModifyDialog.dismiss();
                }
                Toast.makeText(this.context, "网络连接错误", Toast.LENGTH_LONG).show();
                return;
            }
            if (this.count == 3) {
                if (result.equalsIgnoreCase("true") && BackAppActivity.isUnzip()) {
                    BackAppActivity.this.Prodialog.dismiss();
                    BackAppActivity.this.showDownloadDialog(this.CheckedPath, this.serverDir);
            ...
            } else if (this.count == 1 && BackAppActivity.isUnupdate()) {
                ...
                BackAppActivity.this.connect(this.FilePath, this.count + 1, this.CheckedPath, this.serverDir);
            } else if (this.count == 0 && BackAppActivity.isUnzip()) {
                if (BackAppActivity.this.ModifyDialog.isShowing()) {
                    BackAppActivity.this.ModifyDialog.dismiss();
                    BackAppActivity.this.showModifyDialog(result, this.count + 1, this.CheckedPath, this.serverDir);
                }
        ...
        protected void onPreExecute() {
            if (this.count == 1 || this.count == 3) {
                if (!BackAppActivity.this.Prodialog.isShowing()) {
                    BackAppActivity.this.Prodialog.show();
                }
            } else if (this.count == 0) {
                BackAppActivity.this.mProgress.setMax(this.CheckedFile.size());
            }
        }
        ...
        protected void onProgressUpdate(Integer... values) {
            if (this.count == 0 && BackAppActivity.isUnzip()) {
                BackAppActivity.this.mProgressText.setText(values[0] + "/" + values[1]);
                BackAppActivity.this.mProgress.setProgress(values[0]);
            } else if (this.count == 1 && BackAppActivity.isUnupdate()) {
                BackAppActivity.this.Prodialog.setProgress(values[0]);
                BackAppActivity.this.Prodialog.setMessage("正在上传，请稍候...(" + values[0] + "%)");
            }
        }
    }
