...
public class LanuchActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        this.webview = (WebView) this.findViewById(id.webView1);
...
//		}
        ...
        this.path = UIHelper.getShareperference(this, constants.SAVE_INFORMATION,
				"Path", UIHelper.getSoftPath(this)+"/"+ getString(R.string.SD_CARD_TBSAPP_PATH2));
		if (this.path.endsWith("/") == false) {
            this.path += "/";
		}
        ...
        this.handler = new Handler() {
			...
			public void handleMessage(Message msg) {
				if (msg.what == LanuchActivity.THREAD_ERROR_SDCARD) {
					Intent intent = new Intent(LanuchActivity.this,
							SDUnavailableActivity.class);
                    LanuchActivity.this.startActivityForResult(intent, LanuchActivity.ENABLE_SDCARD_REQUEST);
				} else if (msg.what == LanuchActivity.THREAD_ERROR_NONE) {
					if (LanuchActivity.this.launchState.equals(LanuchActivity.this.versionname)) {// 等于true跳转到软件主界面
                        LanuchActivity.this.Startapp();
						Intent mainIntent = new Intent(LanuchActivity.this,
								InitializeToolbarActivity.class);
						if (LanuchActivity.this.getIntent().getExtras() != null) {
							Intent intent = LanuchActivity.this.getIntent();
							int flag = intent.getIntExtra("flag", 0);
							if (flag == 2) {
                        ...
                        String configPath = LanuchActivity.this.getApplicationContext().getFilesDir()
                                .getParentFile().getAbsolutePath();
                        if (configPath.endsWith("/") == false) {
                            configPath = configPath + "/";
                        }
                        String appUserFile = configPath + constants.USER_CONFIG_FILE_NAME;
                        String defapp = LanuchActivity.this.IniFile.getIniString(appUserFile,
                                "system", "default", "tbs-mis", (byte) 0);
                        String dir = LanuchActivity.this.IniFile.getIniString(appUserFile,
                                defapp, "instdir", "TbsMis", (byte) 0);
                        String webRoot = UIHelper.getStoragePath(LanuchActivity.this);
                        String path = webRoot + constants.SD_CARD_TBSSOFT_PATH3 + "/"+ dir;
                        UIHelper.setSharePerference(LanuchActivity.this, constants.SAVE_INFORMATION, "Path",
                                path);
                        LanuchActivity.this.Startapp();
						Intent mainIntent = new Intent(LanuchActivity.this,
		...
		LanuchActivity.MyAsyncTask task = new LanuchActivity.MyAsyncTask(this, UIHelper.getStoragePath(this));// 启动异步加载功能
		task.execute();
	}
	...
	private boolean doDeploy(String assetTbk, String webRootTbk, String packName) {
		String webRoot = UIHelper.getStoragePath(this) + constants.SD_CARD_TBSSOFT_PATH3 + "/";
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == LanuchActivity.ENABLE_SDCARD_REQUEST) {
	...
	class MyAsyncTask extends AsyncTask<Void, Integer, Message> {
		...
		protected Message doInBackground(Void... params) {
			...
			if (false == Environment.getExternalStorageState().equals(
				...
				if (LanuchActivity.this.ShowSplash && timeSleep > 0) {
					try {
				...
				msg.what = LanuchActivity.THREAD_ERROR_SDCARD;
			} else {
				timeSleep = constants.ShowSplashMillisecond
						- (System.currentTimeMillis() - timeStart);
				msg.what = LanuchActivity.THREAD_ERROR_NONE;
                LanuchActivity.this.iniDeploy(constants.USER_CONFIG_FILE_NAME);
                LanuchActivity.this.iniCDeploy(constants.APP_CONFIG_FILE_NAME);
				List<String> list = new ArrayList<String>();
				list = LanuchActivity.this.getfileFromAssets(this.Path);
				for (int i = 0; i < list.size(); i++) {
					if (LanuchActivity.this.launchState.equals("1.0.0")) {
					...
					if (!(webROOT.exists() && webROOT.isDirectory())) {
                        LanuchActivity.this.doDeploy(this.Path + "/" + list.get(i),
		...
		protected void onPostExecute(Message integer) {
            LanuchActivity.this.handler.sendMessage(integer);
			if (LanuchActivity.this.Prodialog.isShowing()) {
                LanuchActivity.this.Prodialog.dismiss();
			}
		}
		...
		protected void onProgressUpdate(Integer... values) {
            LanuchActivity.this.Prodialog.show();
            LanuchActivity.this.Prodialog.setMessage(" 正在安装，请稍候...(" + (values[0] + 1) + "/"
					+ values[1] + ")");
		}
	}
