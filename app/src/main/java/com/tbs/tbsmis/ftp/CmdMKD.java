...
public class CmdMKD extends FtpCmd
{
	...
	@Override
	public void run() {
        this.myLog.l(Log.DEBUG, "MKD executing");
		...
		mainblock: {
			...
			toCreate = FtpCmd.inputPathToChrootedFile(this.sessionThread.getWorkingDir(), param);
			if(this.violatesChroot(toCreate)) {
		...
		if(errString != null) {
            ...
            this.myLog.l(Log.INFO, "MKD error: " + errString.trim());
		} else {
            this.sessionThread.writeString("250 Directory created\r\n");
		}
        this.myLog.l(Log.INFO, "MKD complete");
	}
