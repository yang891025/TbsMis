...
public class CmdLIST extends CmdAbstractListing
{
    ...
    public static final long MS_IN_SIX_MONTHS = 6 * 30 * 24 * 60 * 60 * 1000;
	...
	public void run() {
		...
		mainblock: {
			...
			while(param.startsWith("-")) {
				...
				param = FtpCmd.getParameter(param);
			}
			...
			if(param.equals("")) {
				...
				if(this.violatesChroot(fileToList)) {
			...
			if(fileToList.isDirectory()) {
				...
				errString = this.listDirectory(response, fileToList);
				if(errString != null) {
					break mainblock;
				}
				listing = response.toString();
			} else {
				listing = this.makeLsString(fileToList);
				if(listing == null) {
					errString = "450 Couldn't list that file\r\n";
					break mainblock;
				}
			}
			errString = this.sendListing(listing);
			if(errString != null) {
				break mainblock;
			}
		}	
		...
		if(errString != null) {
            ...
            this.myLog.l(Log.DEBUG, "LIST failed with: " + errString);
		} else {
            this.myLog.l(Log.DEBUG, "LIST completed OK");
		}
	...
	protected String makeLsString(File file) {
		...
		if(!file.exists()) {
            CmdAbstractListing.staticLog.l(Log.INFO, "makeLsString had nonexistent file");
			return null;
		}
		...
		{
            CmdAbstractListing.staticLog.l(Log.INFO, "Filename omitted due to disallowed character");
			return null;
		} else {
		...
		if(System.currentTimeMillis() - mTime > CmdLIST.MS_IN_SIX_MONTHS) {
