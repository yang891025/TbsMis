...
public abstract class CmdAbstractListing extends FtpCmd {
	...
	public String listDirectory(StringBuilder response, File dir) {
        ...
        this.myLog.l(Log.DEBUG, "Listing directory: " + dir);
        ...
        this.myLog.l(Log.DEBUG, "Dir len " + entries.length);
		for(File entry : entries) {
			String curLine = this.makeLsString(entry);
			if(curLine != null) {
	...
	protected String sendListing(String listing) {
		if(this.sessionThread.startUsingDataSocket()) {
            this.myLog.l(Log.DEBUG, "LIST/NLST done making socket");
		} else {
            this.sessionThread.closeDataSocket();
			return "425 Error opening data socket\r\n";
		}
		String mode = this.sessionThread.isBinaryMode() ? "BINARY" : "ASCII";
        this.sessionThread.writeString(
				"150 Opening "+mode+" mode data connection for file list\r\n");
        this.myLog.l(Log.DEBUG, "Sent code 150, sending listing string now");
		if(!this.sessionThread.sendViaDataSocket(listing)) {
            this.myLog.l(Log.DEBUG, "sendViaDataSocket failure");
            this.sessionThread.closeDataSocket();
			return "426 Data socket or network error\r\n";
		}
        this.sessionThread.closeDataSocket();
        this.myLog.l(Log.DEBUG, "Listing sendViaDataSocket success");
