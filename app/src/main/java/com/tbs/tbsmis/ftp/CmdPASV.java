...
public class CmdPASV extends FtpCmd
{
	...
	public void run() {
        ...
        this.myLog.l(Log.DEBUG, "PASV running");
		int port;
		if((port = this.sessionThread.onPasv()) == 0) {
			// There was a problem opening a port
            this.myLog.l(Log.ERROR, "Couldn't open a port for PASV");
		...
		if(addr == null) {
            this.myLog.l(Log.ERROR, "PASV IP string invalid");
            this.sessionThread.writeString(cantOpen);
			return;
		}
        this.myLog.d("PASV sending IP: " + addr.getHostAddress());
		if(port < 1) {
            this.myLog.l(Log.ERROR, "PASV port number invalid");
            this.sessionThread.writeString(cantOpen);
			return;
		}
        ...
        this.myLog.l(Log.DEBUG, "PASV completed, sent: " + responseString);
	}
}
