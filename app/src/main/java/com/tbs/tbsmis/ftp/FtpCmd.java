...
public abstract class FtpCmd implements Runnable {
	...
	protected static void dispatchCommand(SessionThread session, 
		...
		if(strings == null) {
            ...
            FtpCmd.staticLog.l(Log.INFO, errString);
			session.writeString(errString);
			return;
		}
		if(strings.length < 1) {
            FtpCmd.staticLog.l(Log.INFO, "No strings parsed");
			session.writeString(unrecognizedCmdMsg);
			return;
		}
		String verb = strings[0];
		if(verb.length() < 1) {
            FtpCmd.staticLog.l(Log.INFO, "Invalid command verb");
			session.writeString(unrecognizedCmdMsg);
			return;
		}
		...
		FtpCmd cmdInstance = FtpCmd.getCmd(verb, session, inputString);

		if(cmdInstance == null) {
			// If we couldn't find a matching command,
            FtpCmd.staticLog.l(Log.DEBUG, "Ignoring unrecognized FTP verb: " + verb);
			session.writeString(unrecognizedCmdMsg);
			return;
    ...
    public static String getParameter(String input, boolean silent) {
		...
		if(!silent) {
            FtpCmd.staticLog.l(Log.DEBUG, "Parsed argument: " + retString);
		}
		return retString; 
	}
    ...
    public static String getParameter(String input) {
		return FtpCmd.getParameter(input, false);
	}
	...
	public boolean violatesChroot(File file) {
		...
		try {
			...
			if(!canonicalPath.startsWith(chroot.toString())) {
                this.myLog.l(Log.INFO, "Path violated folder restriction, denying");
                this.myLog.l(Log.DEBUG, "path: " + canonicalPath);
                this.myLog.l(Log.DEBUG, "chroot: " + chroot);
				return true; // the path must begin with the chroot path
			}
			return false;
		} catch(Exception e) {
            this.myLog.l(Log.INFO, "Path canonicalization problem: " + e);
            this.myLog.l(Log.INFO, "When checking file: " + file.getAbsolutePath());
