...
/**
 * Created by TBS on 2017/8/16.
 */

public class DiskPopMenus
{
    ...
    public DiskPopMenus(WebView webview, Context context, JSONArray _jsonArray,
        ...
        for (int i = 0; i < this.jsonArray.length(); i++) {
            ...
            layoutItem.setOnClickListener(new View.OnClickListener()
            {
                ...
                public void onClick(View v) {
                    ...
                    if (DiskPopMenus.this.type.equalsIgnoreCase("view")) {
                        try {
                            DiskPopMenus.this.webview.loadUrl(ob.getString("url"));
                        } catch (JSONException e) {
                            // TODO Auto-generated catch block
                            e.printStackTrace();
                        }
                    } else if (DiskPopMenus.this.type.equalsIgnoreCase("button")) {
                        try {
                            String key = ob.getString("key");
                            ActionBar bar = ((FileExplorerTabActivity) DiskPopMenus.this.context).getActionBar();
                            if (bar.getSelectedNavigationIndex() != Util.SDCARD_TAB_INDEX) {
                                if (key.equalsIgnoreCase("rename")) {
                                    ((DiskManagerActivity) ((FileExplorerTabActivity) DiskPopMenus.this.context)
                                ...
                                if (key.equalsIgnoreCase("rename")) {
                                    ((FileViewActivity) ((FileExplorerTabActivity) DiskPopMenus.this.context)
                        ...
                        Toast.makeText(DiskPopMenus.this.context, "类型不支持", Toast.LENGTH_SHORT).show();
                        DiskPopMenus.this.dismiss();
                    }
                    DiskPopMenus.this.dismiss();
                }
            });
