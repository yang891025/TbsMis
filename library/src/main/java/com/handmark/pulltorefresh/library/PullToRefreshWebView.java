...
public class PullToRefreshWebView extends PullToRefreshBase<WebView> {
	...
	public PullToRefreshWebView(Context context) {
		 ...
		 */
        this.setOnRefreshListener(PullToRefreshWebView.defaultOnRefreshListener);
	...
	protected boolean isReadyForPullEnd() {
		float exactContentHeight = (float) Math.floor(this.mRefreshableView.getContentHeight() * this.mRefreshableView.getScale());
				//FloatMath.floor();
	...
	final class InternalWebViewSDK9 extends WebView {
		...
		protected boolean overScrollBy(int deltaX, int deltaY, int scrollX, int scrollY, int scrollRangeX,
			...
			OverscrollHelper.overScrollBy(PullToRefreshWebView.this, deltaX, scrollX, deltaY, scrollY,
                    this.getScrollRange(), InternalWebViewSDK9.OVERSCROLL_FUZZY_THRESHOLD, InternalWebViewSDK9.OVERSCROLL_SCALE_FACTOR, isTouchEvent);

			return returnValue;
		}

		private int getScrollRange() {
			return (int) Math.max(0, (float) Math.floor(PullToRefreshWebView.this.mRefreshableView.getContentHeight() * PullToRefreshWebView
