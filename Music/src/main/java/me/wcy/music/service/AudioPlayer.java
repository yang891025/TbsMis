...
public class AudioPlayer
{
    ...
    public void addAndPlay(List<Music> music, int position) {
        musicList.clear();
        DBManager.get().getMusicDao().deleteAll();
        musicList.addAll(music);
        DBManager.get().getMusicDao().insertInTx(music);
        play(position);
    }
    ...
    public void play(int position) {
        ...
        if (music.getType() == Music.Type.ONLINE) {
            new PlayOnlineMusic(context, music)
            {
                @Override
    ...
    public void seekTo(int msec) {
        if (isPlaying() || isPausing()) {
            ...
            MediaSessionManager.get().updatePlaybackState();
            for (OnPlayerEventListener listener : listeners) {
                listener.onPublish(mediaPlayer.getDuration(),msec);
            }
        }
    }
    ...
    private Runnable mPublishRunnable = new Runnable()
    {
        ...
        public void run() {
            if (isPlaying()) {
                for (OnPlayerEventListener listener : listeners) {
                    listener.onPublish(mediaPlayer.getDuration(),mediaPlayer.getCurrentPosition());
                }
            }
